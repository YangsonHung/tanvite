...
function resolveImportTarget(sourceFile, specifier, projectRoot = process.cwd()) {
...
function checkImportBoundary(sourceFile, specifier, projectRoot = process.cwd()) {
...
async function main() {
  const sourceRoot = path.join(process.cwd(), SRC_ROOT);
  ...
  for (const filePath of files) {
    ...
    for (const match of source.matchAll(IMPORT_PATTERN)) {
      ...
      const violation = checkImportBoundary(filePath, specifier);

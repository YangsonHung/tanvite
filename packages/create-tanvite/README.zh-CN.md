...
| `--<feature>` / `--no-<feature>` | 启用/禁用单个特性。支持：`openspec`、`openapi`、`playwright`、`pages`、`agents`、`hooks`、`lint-file-naming`、`lint-max-lines`。 |
| `--max-lines <N>` | `lint-max-lines` 检查的单文件行数上限；必须是介于 `100` 与 `1000` 之间的整数（含两端）。默认：`300`。 |
| `--hooks-agents <claude,codex>` | 为哪些代理生成钩子（需配合 `--hooks`）。逗号分隔。`-y` 模式默认：`claude,codex`。 |
